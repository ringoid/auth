...
func init() {
	...
	anlogger, err = syslog.New(papertrailAddress, fmt.Sprintf("%s-%s", env, "internal-complete-auth"))
	if err != nil {
...
func handler(ctx context.Context, request apimodel.VerifyReq) (apimodel.VerifyResp, error) {
	lc, _ := lambdacontext.FromContext(ctx)

	anlogger.Debugf(lc, "complete.go : handle request %v", request)

	userInfo, ok, errStr := fetchBySessionId(request.SessionId, lc)
	if !ok {
		anlogger.Errorf(lc, "complete.go : userId [%s], return %s to client", userInfo.UserId, errStr)
		return apimodel.VerifyResp{}, nil
	}
	...
	anlogger.Debugf(lc, "complete.go : return respone=%v to client, userId [%s]", resp, userInfo.UserId)
	return resp, nil
}
