...
func init() {
	...
	anlogger.Debugf(nil, "internal_handle_task.go : cloudwatch client was successfully initialized")

	baseCloudWatchNamespace, ok = os.LookupEnv("BASE_CLOUD_WATCH_NAMESPACE")
	if !ok {
		anlogger.Fatalf(nil, "internal_handle_task.go : env can not be empty BASE_CLOUD_WATCH_NAMESPACE")
	}
	anlogger.Debugf(nil, "internal_handle_task.go : start with BASE_CLOUD_WATCH_NAMESPACE = [%s]", baseCloudWatchNamespace)

	nexmoMetricName, ok = os.LookupEnv("CLOUD_WATCH_NEXMO_NOT_COMPLETE_VERIFICATION_IN_TIME_METRIC_NAME")
	if !ok {
		anlogger.Fatalf(nil, "internal_handle_task.go : env can not be empty CLOUD_WATCH_NEXMO_NOT_COMPLETE_VERIFICATION_IN_TIME_METRIC_NAME")
	}
	anlogger.Debugf(nil, "internal_handle_task.go : start with CLOUD_WATCH_NEXMO_NOT_COMPLETE_VERIFICATION_IN_TIME_METRIC_NAME = [%s]", nexmoMetricName)

	twilioMetricName, ok = os.LookupEnv("CLOUD_WATCH_TWILIO_NOT_COMPLETE_VERIFICATION_IN_TIME_METRIC_NAME")
	if !ok {
		anlogger.Fatalf(nil, "internal_handle_task.go : env can not be empty CLOUD_WATCH_TWILIO_NOT_COMPLETE_VERIFICATION_IN_TIME_METRIC_NAME")
	}
	anlogger.Debugf(nil, "internal_handle_task.go : start with CLOUD_WATCH_TWILIO_NOT_COMPLETE_VERIFICATION_IN_TIME_METRIC_NAME = [%s]", twilioMetricName)
}
